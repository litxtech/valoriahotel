-- Aktif misafirler (silinmemiş, banlı değil) en başta; avatarları ile birlikte.
...
BEGIN
  ...
  ORDER BY
    -- Aktif (silinmemiş, banlı değil) en başta
    (g.deleted_at IS NULL AND (g.banned_until IS NULL OR g.banned_until <= now())) DESC,
    g.created_at DESC;
END;
$$;
