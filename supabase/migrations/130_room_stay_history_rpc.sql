-- Personel: oda bazında sözleşme onayları + misafir bilgisi (oda durumu ekranı için)
-- RLS contract_acceptances satır bazında kısıtlı olduğundan SECURITY DEFINER ile tek kaynak.
...
COMMENT ON FUNCTION public.get_room_stay_history(UUID) IS
  'Aktif personel için: odada sözleşmesi bu odaya atanmış tüm kayıtlar (misafir + şablon + atanan personel).';

GRANT EXECUTE ON FUNCTION public.get_room_stay_history(UUID) TO authenticated;
