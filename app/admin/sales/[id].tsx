...
export default function AdminSaleDetailScreen() {
  ...
  if (!canUse) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.deniedDesc}>Bu ekran için admin, resepsiyon şefi veya satış yetkisi gerekir.</Text>
      </View>
    );
  }
  ...
  if (!sale) {
    return (
      <View style={styles.center}>
        <Ionicons name="alert-circle-outline" size={26} color={adminTheme.colors.textMuted} />
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.card}>
        ...
        <View style={styles.split}>
          ...
          <View style={[styles.pill, { backgroundColor: adminTheme.colors.surfaceTertiary }]}>
            <Text style={[styles.pillText, { color: adminTheme.colors.text }]}>{sale.source_type}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.actionsRow}>
        <TouchableOpacity
          ...
          disabled={saving}
        >
          <Ionicons name="refresh" size={18} color={adminTheme.colors.accent} />
...
const styles = StyleSheet.create({
  ...
  sub: { marginTop: 4, color: adminTheme.colors.textMuted, fontSize: 12, fontWeight: '700' },
  split: { flexDirection: 'row', gap: 8, marginTop: 10, flexWrap: 'wrap' },
  pill: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 999, backgroundColor: adminTheme.colors.primary },
  pillText: { fontSize: 11, fontWeight: '900', color: '#fff' },
  sectionTitle: { marginTop: 10, marginBottom: 8, fontSize: 15, fontWeight: '900', color: adminTheme.colors.text },
  row: { flexDirection: 'row', justifyContent: 'space-between', gap: 12, paddingVertical: 8 },
  rowLabel: { fontSize: 12, fontWeight: '800', color: adminTheme.colors.textMuted, flex: 1 },
  rowValue: { fontSize: 13, fontWeight: '900', color: adminTheme.colors.text, flex: 1, textAlign: 'right' },
  hr: { height: 1, backgroundColor: adminTheme.colors.border, marginVertical: 10 },
  note: { marginTop: 10, fontSize: 12, color: adminTheme.colors.textMuted, lineHeight: 18 },
  ...
  actionBtnPrimary: { backgroundColor: adminTheme.colors.primary },
  actionBtnPrimaryText: { color: '#fff', fontWeight: '900' },
  actionBtnGhost: { backgroundColor: adminTheme.colors.surface, borderWidth: 1, borderColor: adminTheme.colors.border },
  actionBtnGhostText: { color: adminTheme.colors.accent, fontWeight: '900' },
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 20, backgroundColor: adminTheme.colors.surfaceSecondary },
  deniedTitle: { marginTop: 10, fontSize: 16, fontWeight: '900', color: adminTheme.colors.text },
  deniedDesc: { marginTop: 6, fontSize: 13, color: adminTheme.colors.textMuted, textAlign: 'center' },
  backBtn: { marginTop: 14, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, backgroundColor: adminTheme.colors.primary },
