/**
 * Misafir profil sayfası - akıştaki "Tüm kullanıcılar" strip'inden tıklanınca.
 * Personel profili gibi avatar, isim gösterir.
 */
...
export default function GuestProfileScreen() {
  ...
  const loadGuest = useCallback(async () => {
    ...
    if (guestRow?.guest_id) {
      ...
      if (hidden.hiddenGuestIds.has(id)) {
        setGuest(null);
        setLoading(false);
        return;
      }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 24 }]}>
      ...
      <View style={styles.profileBlock}>
        ...
        >
          {guest.photo_url ? (
            <CachedImage uri={guest.photo_url} style={styles.avatarSmall} contentFit="cover" />
          ) : (
            <View style={[styles.avatarSmall, styles.avatarPlaceholder]}>
              <Text style={styles.avatarLetterSmall}>{displayName.charAt(0).toUpperCase()}</Text>
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={styles.postsSection}>
          {posts.map((p) => (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/customer/feed/${p.id}`)}
              activeOpacity={0.85}
            >
              ...
              <View style={styles.postInfo}>
                <Text style={styles.postTitle} numberOfLines={1}>{p.title || (p.media_type === 'video' ? 'Video' : 'Fotoğraf')}</Text>
...
const styles = StyleSheet.create({
  ...
  backBtnText: { fontSize: 15, fontWeight: '600', color: theme.colors.white },
});
